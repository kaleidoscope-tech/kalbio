...
from kalbio.entity_fields import EntityField
...
def test_export_data_keeps_raw_id_for_colliding_names(
    mocker: MockerFixture, kal_client_mock: KaleidoscopeClient
):
    """
    Test that export_data():
    - Does not collapse two fields that share a name into one output key, which
      would silently drop one value. Colliding names keep the raw field id.
    """
    key_field = EntityField.model_validate({"id": "kf-1", "field_name": "shared"})
    data_field = EntityField.model_validate({"id": "df-1", "field_name": "shared"})

    mocker.patch.object(
        kal_client_mock.entity_fields, "get_key_fields", return_value=[key_field]
    )
    mocker.patch.object(
        kal_client_mock.entity_fields, "get_data_fields", return_value=[data_field]
    )

    input_data = [{"kf-1": "key-value", "df-1": "data-value"}]

    result = export_data(kal_client_mock, input_data)

    # Both values survive because the colliding name is not used as a key.
    assert result == [{"kf-1": "key-value", "df-1": "data-value"}]
