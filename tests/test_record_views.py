...
def test_get_data_fields_on_view_returns_resolved_fields(
    mocker: MockerFixture, kal_client_mock: KaleidoscopeClient
):
    """
    Test that RecordViewsService.get_data_fields_on_view():
    - Returns DataField objects for each data_field_id on the view
    - Skips view_fields with no data_field_id (e.g. lookup-only fields)
    - Optionally filters by field_type
    """
    view_id = _MockData.RECORD_VIEWS[0]["id"]

    mocker.patch.object(
        kal_client_mock.record_views,
        "get_record_views",
        return_value=kal_client_mock.record_views._create_record_views_list(
            _MockData.RECORD_VIEWS
        ),
    )

    def fake_get_data_field_by_id(field_id):
        # Return a STATUS DataField for the first known id, a TEXT for the rest.
        fake = mocker.MagicMock()
        fake.id = field_id
        first_data_field_id = _MockData.RECORD_VIEWS[0]["view_fields"][0]["data_field_id"]
        fake.field_type = (
            DataFieldTypeEnum.STATUS
            if field_id == first_data_field_id
            else DataFieldTypeEnum.TEXT
        )
        return fake
    ...
    mocker.patch.object(
        ...
        side_effect=fake_get_data_field_by_id,
    )

    all_fields = kal_client_mock.record_views.get_data_fields_on_view(view_id)
    expected_count = sum(
        1 for vf in _MockData.RECORD_VIEWS[0]["view_fields"] if vf.get("data_field_id")
    )
    assert len(all_fields) == expected_count

    status_fields = kal_client_mock.record_views.get_data_fields_on_view(
        view_id, DataFieldTypeEnum.STATUS
    )
    assert len(status_fields) == 1
    assert status_fields[0].field_type == DataFieldTypeEnum.STATUS


def test_get_data_fields_on_view_returns_empty_for_unknown_view(
    mocker: MockerFixture, kal_client_mock: KaleidoscopeClient
):
    """
    Test that RecordViewsService.get_data_fields_on_view():
    - Returns an empty list when the view_id is not found
    """
    mocker.patch.object(
        kal_client_mock.record_views, "get_record_views", return_value=[]
    )

    result = kal_client_mock.record_views.get_data_fields_on_view("unknown-view-id")
