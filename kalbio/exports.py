...
class ExportsService(_BaseService):
    """Service class for handling data exports from Kaleidoscope.
    ...
    def pull_data(
        ...
        record_set_ids: Optional[str] = None,
        ...
        Args:
            ...
            download_path (str, optional): The directory path where the CSV file will be saved. Created if it does not exist. Defaults to None (uses the system temp directory).
            ...
            record_set_ids (str, optional): The IDs of the record sets to filter records. Defaults to None.
        ...
        dest_dir = Path(download_path) if download_path else Path(tempfile.gettempdir())
        dest_dir.mkdir(parents=True, exist_ok=True)

        url = "/records/export/csv"
        file = self._client._get_file(
            url,
            # Reduce to a bare name so an absolute or "../"-containing filename
            # can't write outside dest_dir.
            str(dest_dir / Path(filename).name),
            params,
        )
